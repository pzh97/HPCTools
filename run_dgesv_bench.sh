#!/bin/bash
#SBATCH --job-name=dgesv_gcc10
#SBATCH --output=results_gcc10_%j.txt
#SBATCH --ntasks=1
...
RESULTS="dgesv_results_gcc10.csv"
RAW_RESULTS="dgesv_results_gcc10_raw.csv"
...
echo "=== Running DGESV Benchmarks (GCC 10.1.0) ==="
...
for flag in "${OPT_FLAGS[@]}"; do
    ...
    bin="dgesv_bench_gcc10_${flag}_${JOB_TAG}"
    ...
    case $flag in
        ...
        "O3-vec")
            "$CC" -O3 -march=native -ftree-vectorize \
                -fopt-info-vec-optimized="vec_gcc10_O3.txt" \
                -fopt-info-vec-missed="missed_gcc10_O3.txt" \
                dgesv.c timer.c main.c -o "$bin" -lopenblas -lm
            ;;
        "Ofast-vec")
            "$CC" -Ofast -march=native -ftree-vectorize \
                -fopt-info-vec-optimized="vec_gcc10_Ofast.txt" \
                -fopt-info-vec-missed="missed_gcc10_Ofast.txt" \
                dgesv.c timer.c main.c -o "$bin" -lopenblas -lm
            ;;
    esac
...
rm -f dgesv_bench_gcc10_*_"$JOB_TAG"
